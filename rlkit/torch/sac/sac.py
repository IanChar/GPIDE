...
class SACTrainer(TorchTrainer, LossFunction):
    def __init__(
        ...
        self.policy_kl_beta = policy_kl_beta
        self.q_kl_beta = q_kl_beta
    ...
    def compute_loss(
        ...
        assert self.policy_kl_beta <= 0.0, 'TODO'
        assert self.q_kl_beta <= 0.0, 'TODO'
        ...
        if self.use_automatic_entropy_tuning:
            current_log_probs = log_pi.mean().item()
    ...
    def training_mode(self, train):
        ...
        self.qf1.train(train)
        self.qf2.train(train)
